function(input, output) {
  ...
  output$u_generaldescription_datatable1 <- renderTable({
  ...
  output$u_generaldescription_text2 <- renderText({
  ...
  output$u_generaldescription_datatable2 <- renderDataTable({
  ...
  output$u_generaldescription_image2 <- renderImage({
  ...
  output$u_globalview_plot <- renderPlot({
    if (input$u_globalview_radio == "Submission by time") {
    ...
    else if (input$u_globalview_radio == "Number of standardized attempts") {
  ...
  output$u_globalview_caption <- renderText({
    if (input$u_globalview_radio == "Submission by time") {
  ...
  output$u_globalscore_plot <- renderPlot(
  ...
  output$u_numberofattempts_plotly <- renderPlotly({
    sdd_dt %>.%
      filter(.,
        tuto_label == input$u_numberofattempts_selectinput) %>.%
    ...
    # Choix de l'unité
    yvar = df$pct
    if ("percentage (%)" %in% input$u_numberofattempts_radio) {
      yvar = df$pct
      y_axis_name = "Percentage"
      info_tooltip = paste(df$pct, "%", sep = "")
    }
    if ("number of students" %in% input$u_numberofattempts_radio) {
      yvar = df$Freq
  ...
  output$u_numberofattempts_datatable1.1 <- renderDT({
    # Création colonne "count" = nombre de tentative
    sdd_dt %>.%
      filter(., tuto_label == input$u_numberofattempts_selectinput) %>.%
      group_by(., tuto_label, user_name ) %>.%
      summarise(., count = n() ) %>.%
      arrange(., user_name)  %>.%
      ungroup(.) %>.%
  ...
  output$u_students_plotly1.3 <- renderPlotly({
    sdd_dt %>.%
